...
class ConfigManagerDialog(QDialog):
    ...
    def add_core_node(self):
        ...
        if not selected_items:
            # Check if there's only one SOC available
            if self.core_tree.topLevelItemCount() == 1:
                selected_item = self.core_tree.topLevelItem(0)
            else:
        ...
        # Note: CorePropertiesDialog should handle disabling "Is QNX" when OS Type is AUTOSAR
        # and disabling "Is Autosar Compliant" when OS Type is QNX
        core_dialog = CorePropertiesDialog(self)

        # Connect to OS type change signals to handle the dependencies
        if hasattr(core_dialog, "os_type_combo") and hasattr(core_dialog, "is_qnx_checkbox") and hasattr(core_dialog, "is_autosar_checkbox"):
            core_dialog.os_type_combo.currentTextChanged.connect(lambda text: self._update_os_dependencies(core_dialog, text))
            # Initialize state based on default OS selection
            self._update_os_dependencies(core_dialog, core_dialog.os_type_combo.currentText())
    ...
    def _update_os_dependencies(self, dialog, os_type):
        """Update checkbox states based on OS type selection"""
        if os_type.upper() == "AUTOSAR":
            dialog.is_qnx_checkbox.setChecked(False)
            dialog.is_qnx_checkbox.setEnabled(False)
            dialog.is_autosar_checkbox.setEnabled(True)
        elif os_type.upper() == "QNX":
            dialog.is_autosar_checkbox.setChecked(False)
            dialog.is_autosar_checkbox.setEnabled(False)
            dialog.is_qnx_checkbox.setEnabled(True)
        else:
            # For other OS types, enable both options
            dialog.is_qnx_checkbox.setEnabled(True)
            dialog.is_autosar_checkbox.setEnabled(True)
